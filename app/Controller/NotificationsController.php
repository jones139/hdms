 ...
 *                if a user_id named parameter is provided, it lists only 
 *                  the active notifications for the given user_id.
 *
 * @return void
 */
    ...
    public function view($id = null) {
        ...
        $options = array('conditions' => array(
            'Notification.' . $this->Notification->primaryKey => $id));
        $this->set('notification', $this->Notification->find(
            'first', $options));
    }
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Notification->save($this->request->data)) {
                $this->Session->setFlash(
                    __('The notification has been saved.'));
                return $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(
                    __('The notification could not be saved. Please, try again.'));
            }
        }
 ...
 */
    public function edit($id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $options = array('conditions' => array(
                'Notification.' . $this->Notification->primaryKey => $id));
            $this->request->data = $this->Notification->find(
                'first', $options);
        }
        $users = $this->Notification->User->find('list');
        $revisions = $this->Notification->Revision->find('list');
